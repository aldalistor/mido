...
function registerDatabaseHandlers() {
  ...
  ipcMain.handle('admin:reset-database', async () => { requirePermission('MANAGE_DATABASE'); const result = await db.resetDatabase(); currentSession = null; return result; });
  ...
  ipcMain.handle('db:get-invoice', (_event, payload = {}) => { requirePermission('VIEW_REPORTS'); return db.getInvoice(payload.invoiceId); });
  ipcMain.handle('db:update-invoice', async (_event, payload = {}) => { requirePermission('CREATE_INVOICES'); requirePermission('POST_JOURNALS'); const result = await db.updateInvoice({ ...payload, userId: currentSession.userId }); await db.recordAudit({ ...(currentSession.context || {}), userId: currentSession.userId, actionCode: 'UPDATE_INVOICE', entityType: 'INVOICE', entityId: result.invoiceId, afterValue: result }); return result; });
  ipcMain.handle('db:void-invoice', async (_event, payload = {}) => { requirePermission('CREATE_INVOICES'); const result = await db.voidInvoice(payload.invoiceId); await db.recordAudit({ ...(currentSession.context || {}), userId: currentSession.userId, actionCode: 'VOID_INVOICE', entityType: 'INVOICE', entityId: result.invoiceId, afterValue: result }); return result; });
