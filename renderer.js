...
function renderDatabaseWorkspace() {
  ...
  $('db-test').addEventListener('click', async () => { const result = $('db-test-result'); result.className = 'db-test-result checking'; result.textContent = 'جارٍ فحص الاتصال وصلاحيات قاعدة البيانات عبر Backend...'; logDb(`بدء اختبار ${engine.toUpperCase()}`, 'checking'); try { const info = await window.onyxAPI.setupTestConnection(dbPayload()); result.className = 'db-test-result success'; result.textContent = `✓ تم الاتصال بـ ${info.engine} · المستخدم ${info.DB_USER || 'تم التحقق'}.`; logDb(`نجح الاتصال بـ ${info.engine.toUpperCase()}`, 'success'); $('current-db-name').textContent = info.engine.toUpperCase(); $('current-db-detail').textContent = 'تم اختبار الاتصال بنجاح'; } catch (error) { result.className = 'db-test-result error'; result.textContent = `✕ ${error.message}`; logDb(error.message, 'error'); } });
