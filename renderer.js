...
let sessionPermissions = new Set(['ALL']);
let licenseState = { valid: true, readOnly: false };

function hasUiPermission(permission) {
  const required = String(permission || '').toUpperCase();
  return sessionPermissions.has('ALL') || sessionPermissions.has(required) || sessionPermissions.has('MANAGE_USERS');
}

function canWriteUi(permission) {
  if (licenseState.readOnly) { showToast('الترخيص منتهٍ؛ النظام في وضع القراءة فقط.'); return false; }
  if (!hasUiPermission(permission)) { showToast('لا تملك صلاحية تنفيذ هذه العملية.'); return false; }
  return true;
}

function applyCommercialAccess() {
  document.body.classList.toggle('license-read-only', Boolean(licenseState.readOnly));
  document.querySelectorAll('[data-write-permission]').forEach(element => { element.disabled = Boolean(licenseState.readOnly) || !hasUiPermission(element.dataset.writePermission); });
  let banner = $('license-readonly-banner');
  if (licenseState.readOnly && !banner) { banner = document.createElement('div'); banner.id = 'license-readonly-banner'; banner.className = 'license-readonly-banner'; banner.textContent = 'الترخيص منتهٍ أو غير صالح — يمكنك عرض التقارير وتصدير البيانات فقط بعد التجديد.'; document.body.prepend(banner); }
  if (!licenseState.readOnly && banner) banner.remove();
}
...
function renderPurchasesWorkspace() {
  ...
  $('new-supplier-payment').addEventListener('click', () => { if (canWriteUi('RECORD_PAYMENTS')) openPaymentForm('purchase'); });
...
function renderSalesWorkspace() {
  ...
  $('new-customer-receipt').addEventListener('click', () => { if (canWriteUi('RECORD_PAYMENTS')) openPaymentForm('sale'); });
...
async function renderReportsWorkspace() {
  ...
  const load = async () => { const filters = { startDate: $('report-start').value, endDate: $('report-end').value }; if (dataMode === 'oracle' && window.onyxAPI?.financialReports) { try { reportData = await window.onyxAPI.financialReports(filters); } catch (error) { console.error('Financial report load failed:', error); showToast('تعذر تحميل التقرير من Oracle.'); reportData = null; } } else reportData = demoData(); if (reportData) { updateKpis(reportData); renderReportContent(activeReport, reportData); } };
...
function openForm(view) {
  ...
  if (view === 'sales' || view === 'purchases') {
    ...
    $('modal-fields').insertAdjacentHTML('beforeend', '<label>حالة السداد<select name="paymentStatus"><option value="UNPAID">غير مدفوعة</option><option value="PARTIAL">دفعة جزئية</option><option value="PAID">مدفوعة</option></select></label><label>المبلغ المسدد<input name="paidAmount" type="number" min="0" step="0.01" value="0" /></label><label>الخصم العام<input name="discountAmount" type="number" min="0" step="0.01" value="0" /></label><label>معرف الفاتورة الأصلية للمرتجع<input name="sourceInvoiceId" type="number" min="1" /></label><p class="approval-hint">سيتم حفظ الفاتورة كمسودة، ثم تحتاج إلى اعتماد قبل الترحيل.</p>');
...
function openPaymentForm(kind) {
  const form = $('form-modal');
  $('modal-title').textContent = kind === 'sale' ? 'سند قبض من عميل' : 'سند دفع لمورد';
  $('modal-fields').innerHTML = '<label>معرف الفاتورة<input name="invoiceId" type="number" min="1" required /></label><label>المبلغ<input name="amount" type="number" min="0.01" step="0.01" required /></label><label>طريقة الدفع<select name="paymentMethod"><option value="CASH">نقدًا</option><option value="BANK">بنك</option><option value="CARD">بطاقة</option><option value="E_WALLET">محفظة إلكترونية</option></select></label><label>المرجع<input name="reference" /></label>';
  form.dataset.view = 'invoice-payment'; form.dataset.paymentKind = kind; form.classList.add('open');
}

async function approveInvoiceFromUi(view) {
  if (!canWriteUi('APPROVE_INVOICES') || !hasUiPermission('POST_JOURNALS')) return;
  const invoiceId = window.prompt('أدخل معرف الفاتورة المسودة لاعتمادها وترحيلها:');
  if (!invoiceId) return;
  try {
    if (dataMode === 'oracle' && window.onyxAPI) await window.onyxAPI.approveInvoice({ invoiceId: Number(invoiceId) });
    else { const invoice = state.invoices.find(item => String(item.invoiceId || item.ref) === String(invoiceId)); if (invoice) invoice.status = 'مرحّل'; else throw new Error('الفاتورة غير موجودة في الوضع التجريبي.'); save(); }
    renderModule(view); showToast('تم اعتماد الفاتورة وترحيلها بنجاح');
...
$('entry-form').addEventListener('submit', async event => {
  ...
  if (view === 'invoice-payment') {
    try {
      if (!canWriteUi('RECORD_PAYMENTS')) return;
      if (dataMode === 'oracle' && window.onyxAPI) await window.onyxAPI.recordPayment({ invoiceId: Number(data.invoiceId), amount: Number(data.amount), paymentMethod: data.paymentMethod, reference: data.reference });
      else { const invoice = state.invoices.find(item => String(item.invoiceId || item.ref) === String(data.invoiceId)); if (!invoice) throw new Error('الفاتورة غير موجودة في الوضع التجريبي.'); invoice.paidAmount = Number(invoice.paidAmount || 0) + Number(data.amount); invoice.status = invoice.paidAmount >= Number(invoice.total) ? 'مدفوعة' : 'دفعة جزئية'; save(); }
      closeForm(); renderModule(event.currentTarget.dataset.paymentKind === 'sale' ? 'sales' : 'purchases'); showToast('تم تسجيل الدفعة بنجاح');
    } catch (error) { showToast(error.message); }
    return;
  }
  try {
    if ((view === 'sales' || view === 'purchases') && !canWriteUi(data.documentType?.includes('RETURN') ? 'CREATE_RETURNS' : 'CREATE_INVOICES')) return;
    if (dataMode === 'oracle' && window.onyxAPI) {
      ...
      else if (view === 'sales' || view === 'purchases') { const invoiceType = view === 'sales' ? 'SALES_INVOICE' : 'PURCHASE_INVOICE'; if (data.documentType && data.documentType.includes('RETURN')) await window.onyxAPI.createInvoiceReturn({ invoiceId: Number(data.sourceInvoiceId), total: data.lines.reduce((sum, line) => sum + Number(line.quantity || 0) * Number(line.unitPrice || 0), 0), reason: data.documentNote, lines: data.lines }); else if (data.documentType && data.documentType !== invoiceType) await window.onyxAPI.createTradeDocument({ documentType: data.documentType, contactCode: data.partyCode || data.party, currencyCode: data.currency || 'SAR', exchangeRate: data.exchangeRate, documentDate: data.invoiceDate, paymentMethod: data.paymentMethod, dueDate: data.dueDate || null, documentNote: data.documentNote || null, lines: data.lines }); else await window.onyxAPI.createInvoice({ type: view === 'sales' ? 'SALE' : 'PURCHASE', contactCode: data.partyCode || data.party, currency: data.currency || 'SAR', exchangeRate: data.exchangeRate, taxAmount: data.taxAmount, discountAmount: Number(data.discountAmount || 0), paymentMethod: data.paymentMethod, paymentStatus: data.paymentStatus, paidAmount: Number(data.paidAmount || 0), dueDate: data.dueDate || null, invoiceDate: data.invoiceDate, lines: data.lines }); }
    } else {
...
async function completeLogin(session) {
  sessionPermissions = new Set((session.permissions || ['ALL']).map(value => String(value).toUpperCase()));
  licenseState = session.license || (window.onyxAPI?.licenseStatus ? await window.onyxAPI.licenseStatus().catch(() => ({ valid: true, readOnly: false })) : { valid: true, readOnly: false });
