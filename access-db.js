...
async function bootstrapStatus() {
  let initialized = fs.existsSync(currentDbPath());
  if (backend === 'sqlite' && initialized) {
    try {
      const database = require('node:sqlite').DatabaseSync(currentDbPath());
      const table = database.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='ONYX_USER'").get();
      const userCount = table ? Number(database.prepare('SELECT COUNT(*) AS N FROM ONYX_USER').get().N || 0) : 0;
      database.close();
      initialized = userCount > 0;
    } catch (_) { initialized = false; }
  }
  return { initialized, path: currentDbPath(), platform: process.platform, engine: backend, accessAvailable: Boolean(odbc) };
}
...
async function authenticate(username,password){await setup();const normalizedUsername=String(username||'').trim().toUpperCase();const result=rows(await query('SELECT * FROM ONYX_USER WHERE UPPER(USERNAME)=?',[normalizedUsername]));const user=result[0];if(!user)throw new Error(`المستخدم ${normalizedUsername || 'المُدخل'} غير موجود. إذا كان هذا أول تشغيل، أعد التأسيس لإنشاء مستخدم المدير.`);if(!verifyPassword(password,val(user,'PASSWORD_HASH')))throw new Error('كلمة المرور غير صحيحة لهذا المستخدم.');const settings=await organizationSettings();return {userId:Number(val(user,'USER_ID')),username:val(user,'USERNAME'),displayNameAr:val(user,'DISPLAY_NAME_AR'),languageCode:'ar',permissions:['ALL'],roles:[{ROLE_NAME_AR:'مدير النظام'}],context:{companyId:1,branchId:1,fiscalYearId:1},company:{COMPANY_NAME_AR:val(settings.company,'COMPANY_NAME_AR'),BRANCH_NAME_AR:val(settings.branch,'BRANCH_NAME_AR'),FISCAL_YEAR:Number(val(settings.fiscalYear,'FISCAL_YEAR'))}};}
